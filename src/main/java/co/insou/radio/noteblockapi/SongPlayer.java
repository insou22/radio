...
public abstract class SongPlayer {
    ...
    public void addPlayer(Player p) {
        synchronized (this) {
            if (!playerList.contains(p.getName())) {
                ...
                List<SongPlayer> songs = manager.getPlayingSongs().get(p.getName());
                if (songs == null) {
                    songs = new ArrayList<>();
                }
                songs.add(this);
                manager.getPlayingSongs().put(p.getName(), songs);
            }
        }
    }
    ...
    public void removePlayer(Player p) {
        synchronized (this) {
            ...
            if (manager.getPlayingSongs().get(p.getName()) == null) {
                return;
            }
            List<SongPlayer> songs = new ArrayList<>(manager.getPlayingSongs().get(p.getName()));
            songs.remove(this);
            manager.getPlayingSongs().put(p.getName(), songs);
